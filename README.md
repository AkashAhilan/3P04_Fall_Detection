...
A smart fall detection mat using dual load cells (Front side and Back) and an ESP32 microcontroller to detect and alert on potential falls. Built for 3P04 Design Verification.
...
- Add state transitioning logic properly currently once the fall is detected it is stuck in that state (Button to release or time delay possibly)
- Currently the "Key configuration" values noted below need to be changed.
