...
void setup() {
  ...
  Serial.println(
      "Note that the settling time may increase significantly if you use "
      "delay() in your sketch!");
  if (LoadCell.getSPS() < 7) {
    Serial.println(
        "!!Sampling rate is lower than specification, check MCU>HX711 wiring "
        "and pin designations");
  } else if (LoadCell.getSPS() > 100) {
    Serial.println(
        "!!Sampling rate is higher than specification, check MCU>HX711 wiring "
        "and pin designations");
  }
}
...
void loop() {
  ...
  const int serialPrintInterval =
      500;  // increase value to slow down serial print activity
